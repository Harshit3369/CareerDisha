...
app.post("/api/generate-content", async (req, res) => {
    try {
        ...
        let errMsg = (error === null || error === void 0 ? void 0 : error.message) || "Failed to generate content.";
...
app.post("/api/recommend-colleges", async (req, res) => {
    try {
        const { stream, class10Score, class12Score, entranceExams, state, category, courseChosen, budget, } = req.body;
        if (!courseChosen) {
...
`;
                    ...
                    properties: {
                        tiers: {
                            ...
                            items: {
                                type: genai_1.Type.OBJECT,
                                properties: {
                                    tier: { type: genai_1.Type.INTEGER, description: "1, 2, or 3" },
                                    colleges: {
                                        type: genai_1.Type.ARRAY,
                                        items: {
                                            type: genai_1.Type.OBJECT,
                                            properties: {
                                                ...
                                                type: { type: genai_1.Type.STRING, description: "Government | Private | Deemed | Autonomous" },
                                                course: { type: genai_1.Type.STRING },
                                                annualFee: { type: genai_1.Type.STRING },
                                                admissionRoute: { type: genai_1.Type.STRING },
                                                cutoffIndicator: { type: genai_1.Type.STRING },
                                                whyThisCollege: { type: genai_1.Type.STRING },
                                                placementHighlight: { type: genai_1.Type.STRING, description: "Must include average package and lowest package if available." },
                                                reservationBenefit: { type: genai_1.Type.STRING },
                        ...
                        admissionRoadmapSummary: { type: genai_1.Type.STRING },
                        scoreGapAnalysis: {
                            type: genai_1.Type.OBJECT,
                            properties: {
                                tier1Gap: { type: genai_1.Type.STRING },
                                tier2Status: { type: genai_1.Type.STRING },
                                tier3Status: { type: genai_1.Type.STRING },
                                overallVerdict: { type: genai_1.Type.STRING }
                            },
                            required: ["tier1Gap", "tier2Status", "tier3Status", "overallVerdict"]
                        },
                        alternativeCourseSuggestion: { type: genai_1.Type.STRING }
                    },
        ...
        let errMsg = (error === null || error === void 0 ? void 0 : error.message) || "Failed to generate college recommendations.";
        if (errMsg.includes('leaked') || errMsg.includes('API_KEY_INVALID')) {
            errMsg = "Your Gemini API key is missing, invalid, or leaked. Please configure a valid API key in your environment variables.";
        }
        res.status(500).json({ error: errMsg });
...
app.post("/api/exam-tracker", async (req, res) => {
    try {
        const { name, classLevel, stream, state, district, category, gender, score10, score12OrCurrent, annualFamilyIncome, entranceExamsAppeared, interestedCareers, hasNCC, isPhysicallyFit } = req.body;
        let apiKey = process.env.GEMINI_API_KEY || process.env.REACT_APP_GEMINI_API_KEY;
        if (!apiKey || apiKey === 'MY_GEMINI_API_KEY') {
            return res.status(500).json({ error: "Gemini API Key is not configured." });
        }
...
- NEVER return the same exam list regardless of who is asking`;
        const response = await ai.models.generateContent({
            ...
            config: {
                ...
                responseSchema: {
                    type: genai_1.Type.OBJECT,
                    properties: {
                        studentName: { type: genai_1.Type.STRING },
                        stream: { type: genai_1.Type.STRING },
                        classLevel: { type: genai_1.Type.STRING },
                        state: { type: genai_1.Type.STRING },
                        category: { type: genai_1.Type.STRING },
                        totalExamsFound: { type: genai_1.Type.INTEGER },
                        generatedAt: { type: genai_1.Type.STRING },
                        urgentActions: {
                            type: genai_1.Type.ARRAY,
                            items: {
                                type: genai_1.Type.OBJECT,
                                properties: {
                                    examName: { type: genai_1.Type.STRING },
                                    urgentMessage: { type: genai_1.Type.STRING },
                                    daysLeft: { type: genai_1.Type.INTEGER },
                                    applicationLink: { type: genai_1.Type.STRING },
                                    fee: { type: genai_1.Type.STRING },
                                    whyUrgent: { type: genai_1.Type.STRING },
                                },
                        ...
                        primaryExams: {
                            ...
                            items: {
                                type: genai_1.Type.OBJECT,
                                properties: {
                                    examId: { type: genai_1.Type.STRING },
                                    name: { type: genai_1.Type.STRING },
                                    shortName: { type: genai_1.Type.STRING },
                                    conductingBody: { type: genai_1.Type.STRING },
                                    officialWebsite: { type: genai_1.Type.STRING },
                                    examType: { type: genai_1.Type.STRING },
                                    whyRelevant: { type: genai_1.Type.STRING },
                                    dates: {
                                        type: genai_1.Type.OBJECT,
                                        properties: {
                                            applicationOpenDate: { type: genai_1.Type.STRING },
                                            applicationCloseDate: { type: genai_1.Type.STRING },
                                            examDate: { type: genai_1.Type.STRING },
                                            resultDate: { type: genai_1.Type.STRING },
                                            isDateConfirmed: { type: genai_1.Type.BOOLEAN },
                                            tentativeNote: { type: genai_1.Type.STRING }
                                        },
                                    ...
                                    countdown: {
                                        type: genai_1.Type.OBJECT,
                                        properties: {
                                            applicationDaysLeft: { type: genai_1.Type.INTEGER },
                                            examDaysLeft: { type: genai_1.Type.INTEGER },
                                            applicationStatus: { type: genai_1.Type.STRING },
                                            examStatus: { type: genai_1.Type.STRING }
                                        },
                                    ...
                                    eligibility: {
                                        type: genai_1.Type.OBJECT,
                                        properties: {
                                            streams: { type: genai_1.Type.ARRAY, items: { type: genai_1.Type.STRING } },
                                            minScore: { type: genai_1.Type.INTEGER },
                                            maxAge: { type: genai_1.Type.INTEGER },
                                            attempts: { type: genai_1.Type.INTEGER },
                                            studentEligible: { type: genai_1.Type.BOOLEAN },
                                            eligibilityNote: { type: genai_1.Type.STRING }
                                        },
                                    ...
                                    fees: {
                                        type: genai_1.Type.OBJECT,
                                        properties: {
                                            general: { type: genai_1.Type.STRING },
                                            sc_st: { type: genai_1.Type.STRING },
                                            studentFee: { type: genai_1.Type.STRING }
                                        },
                                    ...
                                    scale: {
                                        type: genai_1.Type.OBJECT,
                                        properties: {
                                            totalSeats: { type: genai_1.Type.STRING },
                                            topColleges: { type: genai_1.Type.ARRAY, items: { type: genai_1.Type.STRING } },
                                            difficultyLevel: { type: genai_1.Type.STRING }
                                        },
                                        required: ["totalSeats", "topColleges", "difficultyLevel"]
                                    },
                                    categoryBenefit: { type: genai_1.Type.STRING },
                                    proTip: { type: genai_1.Type.STRING },
                                    documentsNeeded: { type: genai_1.Type.ARRAY, items: { type: genai_1.Type.STRING } }
                                },
                        ...
                        crossStreamOpportunities: {
                            ...
                            items: {
                                ...
                                properties: {
                                    examId: { type: genai_1.Type.STRING },
                                    name: { type: genai_1.Type.STRING },
                                    tag: { type: genai_1.Type.STRING },
                                    surpriseMessage: { type: genai_1.Type.STRING },
                                    dates: {
                                        type: genai_1.Type.OBJECT,
                                        properties: {
                                            applicationOpenDate: { type: genai_1.Type.STRING },
                                    ...
                                    potentialCareer: { type: genai_1.Type.STRING },
                                    proTip: { type: genai_1.Type.STRING }
                                },
                        ...
                        stateSpecificExams: {
                            ...
                            items: {
                                ...
                                properties: {
                                    examId: { type: genai_1.Type.STRING },
                                    name: { type: genai_1.Type.STRING },
                                    importance: { type: genai_1.Type.STRING },
                                    stateAdvantage: { type: genai_1.Type.STRING },
                                    dates: {
                                        type: genai_1.Type.OBJECT,
                                        properties: {
                                            applicationOpenDate: { type: genai_1.Type.STRING },
                        ...
                        futureExams: {
                            ...
                            items: {
                                ...
                                properties: {
                                    examId: { type: genai_1.Type.STRING },
                                    name: { type: genai_1.Type.STRING },
                                    availableAfter: { type: genai_1.Type.STRING },
                                    whyTrackNow: { type: genai_1.Type.STRING }
                                },
                        ...
                        personalizedInsights: {
                            ...
                            items: {
                                type: genai_1.Type.OBJECT,
                                properties: {
                                    insightNumber: { type: genai_1.Type.INTEGER },
                                    title: { type: genai_1.Type.STRING },
                                    message: { type: genai_1.Type.STRING },
                                    actionable: { type: genai_1.Type.STRING },
                                },
                        ...
                        categoryAdvantages: {
                            ...
                            properties: {
                                category: { type: genai_1.Type.STRING },
                                advantages: { type: genai_1.Type.ARRAY, items: { type: genai_1.Type.STRING } },
                                feeWaivers: { type: genai_1.Type.ARRAY, items: { type: genai_1.Type.STRING } },
                                reservationDetails: { type: genai_1.Type.STRING },
                                importantNote: { type: genai_1.Type.STRING }
                            },
                            required: ["category", "advantages", "feeWaivers", "reservationDetails", "importantNote"]
                        },
                        summaryMessage: { type: genai_1.Type.STRING }
                    },
        ...
        let errMsg = (error === null || error === void 0 ? void 0 : error.message) || "Failed to generate exam tracker data.";
        if (errMsg.includes('leaked') || errMsg.includes('API_KEY_INVALID')) {
            errMsg = "Your Gemini API key is missing, invalid, or leaked. Please configure a valid API key in your environment variables.";
        }
        res.status(500).json({ error: errMsg });
...
exports.api = (0, https_1.onRequest)({ invoker: "public", cors: true }, app);
